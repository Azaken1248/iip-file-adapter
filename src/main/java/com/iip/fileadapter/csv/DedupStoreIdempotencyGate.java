 ...
 * idempotency gate (Phase 6.2, made per-contract by Phase 6.3).
 *
 ...
 *
 * <p>The store is chosen by contract, so replaying one contract does not
 * require forgetting what every other contract has been sent.
 */
@Component
public class DedupStoreIdempotencyGate implements IdempotencyGate {

	private final DedupStores dedupStores;

	public DedupStoreIdempotencyGate(DedupStores dedupStores) {
		this.dedupStores = dedupStores;
	}

	@Override
	public boolean shouldApply(RecordEnvelope envelope, Attachment attachment) {
		return !dedupStores.forContract(attachment.contractId()).isProcessed(envelope.recordId());
	}

	@Override
	public void markApplied(RecordEnvelope envelope, Attachment attachment) {
		dedupStores.forContract(attachment.contractId()).markProcessed(envelope.recordId());
	}
}
